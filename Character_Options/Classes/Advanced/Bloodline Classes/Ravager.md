# Ravager

The Ravager is a Yato born warrior who delights in the destruction he sows across the battlefield. Like an unshackled beast, the Ravager shatters enemy lines and human bodies alike, elating himself in the slaughter with a manic glee.

## Requirements

To qualify to become a ravager, a character must fulfill all the following criteria.
*   **Base Attack Bonus**:  +3
*   **Skills**:  Taijutsu 6 ranks.
*   **Feats**:  Advanced Bloodline (Chi no Bousou), Taijutsu Adept.
*   **Special**:  Riot of the Blood ability.

## Class Information

The following information pertains to the Ravager prestige class.

*   **Hit Die**:  d12
*   **Action Points**:  6 + ½ character level, rounded down. CLASS SKILLS The Ravager's class skills are Balance (Dex), Climb (Str), Intimidate (Cha), Jump (Str), Knowledge (tactics) (Int), Listen (Wis), Read/Write Language (none), Speak Language (none), Spot (Wis), Survival (Wis), Swim (Str), Taijutsu (Str), Treat Injury (Wis), Tumble (Dex).
*   **Skill Points**:  3 + Int modifier.

## Table: The Ravager

| Level | Base Attack Bonus | Fort Save | Ref Save | Will Save | Special | Defense Bonus | Reputation Bonus |
| :--- | :---: | :---: | :---: | :---: | :--- | :---: | :---: |
| 1st | +1 | +2 | +1 | +0 | Dreadful Rage | +1 | +0 |
| 2nd | +2 | +3 | +2 | +0 | Monstrous Rage | +2 | +0 |
| 3rd | +3 | +3 | +2 | +1 | Atrocious Rage | +2 | +0 |

## Class Features

The following features pertain to the Ravager prestige class.

### Dreadful Rage

A Ravager owes his combat ability to the brutish strength that runs through his veins. There is however, a kind of unrefined grace in the terrible power that he brings to bear. At 1st level, whenever the Ravager strikes a target as a standard action or deals enough damage to make it drop (typically by dropping it to below 0 hit points or killing it), he gets an immediate, extra melee attack against another creature within reach. The Ravager can use this ability once per round.

### Monstrous Rage

A true Yato embraces the monster lurking within his human personage and learns to revel in the blood he sheds. At 3rd level, the Ravager is no longer limited to using his Dreadful Talent class feature once per round.

### Atrocious Rage

A Yato warrior descends upon the battlefield in a blood haze, leaving nothing but carnage and the shattered forms of his foes in his wake. At 5th level, the Ravager can take a 5-foot step between attacks when using his Dreadful Talent class feature. These 5-foot steps do not count against his movement in the following round.